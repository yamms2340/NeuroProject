...
export default function TaskCard({ task, toggleStatus, deleteTask, toggleStar, editTask }) {
  ...
  return (
    <div className={`TaskCard ${task.status === "completed" ? "completed-task" : ""}`}>
      {isEditing ? (
        ...
        <>
          <div className="task-content">
            <h3 className="TaskCard-header">{task.title}</h3>
          ...
          <div className="TaskCard-actions">
            <button
              ...
              className={`TaskCard-action-button ${task.status === "completed" ? "TaskCard-action-button-status" : ""}`}
            >
              {task.status === "completed" ? "✓ Done" : "Pending"}
            </button>

            <div className="action-buttons">
              <button onClick={() => toggleStar(task.id)} className="TaskCard-action-button-star">
                {task.important ? "⭐" : "☆"}
              </button>
              ...
              <button onClick={() => deleteTask(task.id)} className="TaskCard-action-button-delete">
                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
